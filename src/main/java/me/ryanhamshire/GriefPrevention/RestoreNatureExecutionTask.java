...
class RestoreNatureExecutionTask implements Runnable {
	...
	public void run() {
			...
			for (int z = 1; z < this.snapshots[0][0].length - 1; z++) {
				for (int y = this.miny; y < this.snapshots[0].length; y++) {
					...
					if (blockUpdate.typeId != currentBlock.getType()
							|| !blockUpdate.data.equals(currentBlock.getBlockData())) {
		...
		for (int i = 0; i < entities.length; i++) {
			...
			if (!(entity instanceof Player || entity instanceof Animals)) {
				...
				if (!(entity instanceof Hanging)
						|| GriefPrevention.instance.dataStore.getClaimAt(entity.getLocation(), false, null) == null) {
		...
		if (player != null) {
			Claim claim = new Claim(lesserCorner, greaterCorner, null, new ArrayList<String>(), new ArrayList<String>(),
					new ArrayList<String>(), new ArrayList<String>(), null);
			Visualization visualization = Visualization.FromClaim(claim, player.getLocation().getBlockY(),

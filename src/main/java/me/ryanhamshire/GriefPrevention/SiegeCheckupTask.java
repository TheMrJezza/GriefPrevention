...
class SiegeCheckupTask implements Runnable {
	...
	public void run() {
		...
		// again later)
				...
				&& attacker.getLocation().distanceSquared(defender.getLocation()) < 2500) // 50-block radius for chasing
		{
			this.scheduleAnotherCheck();
		}
	...
	private void scheduleAnotherCheck() {
		this.siegeData.checkupTaskID = GriefPrevention.instance.getServer().getScheduler()
				.scheduleSyncDelayedTask(GriefPrevention.instance, this, 20L * 30);
	}
}
